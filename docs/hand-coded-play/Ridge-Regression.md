...
## Ridge(L2) Regression
- It comes when the sample sizes are relatively small, the Ridge Regression can improve predictions made from new data(i.e. reduce ariance) by making the predictions less sensitive to the Training Data.
  - This is done by adding the Ridge Regression Penalty $\lambda$ to the thing that must be minimized:
$$
sum(residuals^2) + \lambda * Slope^2
$$


```python
X, y = make_regression(n_samples=100, n_features=4, noise=1, random_state = 42, bias = 100)
```
